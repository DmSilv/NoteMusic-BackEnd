...
// Atualizar as explicações no banco de dados
const updateExplanations = async () => {
  try {
    console.log('🔧 Iniciando atualização de explicações...');
    ...
    for (const quiz of quizzes) {
      ...
      for (let i = 0; i < quiz.questions.length; i++) {
        ...
        // Encontrar a explicação correspondente
        const matchingExplanation = explanations.find(e => 
          e.question.toLowerCase() === question.question.toLowerCase() ||
          question.question.toLowerCase().includes(e.question.toLowerCase().substring(0, 15)) ||
          e.question.toLowerCase().includes(question.question.toLowerCase().substring(0, 15))
        );
        
        if (!matchingExplanation) {
          console.log(`⚠️ Não encontrada explicação para: "${question.question.substring(0, 40)}..."`);
          continue;
        }
        
        // Atualizar a explicação
        console.log(`📝 Atualizando explicação para: "${question.question.substring(0, 40)}..."`);
        quiz.questions[i].explanation = matchingExplanation.explanation;
      ...
      if (quizModified) {
        await quiz.save();
        console.log(`✅ Quiz atualizado com explicações detalhadas: ${quiz.title}`);
        quizzesUpdated++;
      } else {
        console.log(`ℹ️ Nenhuma alteração necessária para: ${quiz.title}`);
      }
    }
    
    console.log('\n📊 Resumo da atualização:');
    console.log(`   Total de quizzes verificados: ${quizzes.length}`);
    console.log(`   Total de questões atualizadas: ${totalQuestionsUpdated}`);
